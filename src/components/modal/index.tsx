...
const Model = ({ car, isOpen, close }: Props) => {
  return ( 
  ...
  <div className="fixed inset-0 bg-black bg-opacity-50 z-20 grid place-items-center">
    ...
    rounded-2xl flex flex-col gap-5 shadow-xl overflow-auto">
        <button className="cursor-pointer p-1 absolute end-1 top-1 z-10
        bg-white rounded-full" onClick={close}>
        ...
        {Object.entries(car).map(([key, value]) => (
            <div className="flex justify-between">
                <h4 className="capitalize">{key.split("_").join(" ")}
                </h4>
